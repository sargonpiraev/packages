# @sargonpiraev/github-mcp-server

MCP server for GitHub REST API

## Install

```bash
npm install -g @sargonpiraev/github-mcp-server
```

## Home

This package lives in the public monorepo:

https://github.com/sargonpiraev/packages/tree/main/packages/github-mcp-server

## Run

```bash
npx @sargonpiraev/github-mcp-server
```

Configure API credentials via environment variables (see `.env.example` when provided).
